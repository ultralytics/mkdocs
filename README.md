...
Both modes support the same configuration options, with two exceptions: `enabled` applies to plugin mode only, and postprocess mode additionally accepts `use_processes` (default `True`) and `workers` (default: CPU count) to control parallelism.
